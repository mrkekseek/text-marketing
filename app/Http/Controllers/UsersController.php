		...
		foreach ($data as $item) {
			...
			foreach ($item['users'] as $row) {
				...
				foreach ($row['lists'] as $old => $list_row) {
					if (! in_array($old, $lists_id)) {
						...
						$list->name = $list_row['name'];
						$list->users_id = $user->id;
						$list->save();

						$new_list_id[$list_row['lists_id']] = $list->id;

						foreach ($item['clients'] as $client_row) {
							if ($client_row['lists_id'] == $old && ! in_array($client_row['phones_id'], $clients)) {
								$client = new Client();
								$client->team_id = $team->id;
								$client->firstname = $client_row['firstname'];
								$client->lastname = $client_row['lastname'];
								$client->phone = $client_row['phone'];
								$client->view_phone = $client_row['view_phone'];
								$client->email = $client_row['email'];
								$client->source =  $client_row['source'];
								$client->save();
			...
			foreach ($item['clients'] as $client_row) {
				...
				foreach ($client_row['seances'] as $seances_row) {
					...
					$seance->completed = $seances_row['completed'];
